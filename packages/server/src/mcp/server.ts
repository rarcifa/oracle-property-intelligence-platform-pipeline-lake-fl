  ...
  switch (message.method) {
    case "initialize": {
      ...
      return ok(id, {
        ...
        instructions: `Query the published Lake County, Florida property table (one row per parcel) and permit table (one row per permit). Call getPropertyQuerySchema first to learn the ${QUERY_TABLE_COLUMN_COUNT} property columns. Use getPropertyPermits for full permit details, queryProperties for arbitrary read-only SQL against the \`properties\` or \`permits\` view, or the purpose-built tools for aged roofs, open roofing permits and radius search. bbb_rating is a real column that is always null because the default BBB route returned HTTP 403 and no approved official-API harvest was run; one prohibited browser-fingerprint spoof returned 200 during verification but supplied no ingested data. contractor_name is published only for parcels in Clermont, one of the county's fifteen permitting jurisdictions, and is null elsewhere: read the row's enrichment_status to tell a gated null (contractor_gated_403) from an established absence (contractor_absent_on_permit), and never report a contractor count as countywide coverage.`,
      });
    }
