...
describe("sync-mcp-json path resolution", () => {
  it("resolves the default MCP path to the repo-root .mcp.json, not a sibling checkout", () => {
...
describe("syncMcpJson against a synthetic mcp.json fixture", () => {
  it("rewrites only the managed env keys and preserves the launcher and unmanaged env", async () => {
    ...
    expect(Object.keys(maps.PROPERTY_QUERY_TABLE_MAP)).toHaveLength(15);
    expect(Object.keys(maps.PERMIT_QUERY_TABLE_MAP)).toHaveLength(5);
    expect(Object.keys(maps.DATASET_COVERAGE_MAP)).toHaveLength(14);
    ...
    expect(Object.keys(JSON.parse(env.PROPERTY_QUERY_TABLE_MAP))).toHaveLength(15);
    expect(Object.keys(JSON.parse(env.PERMIT_QUERY_TABLE_MAP))).toHaveLength(5);
    expect(Object.keys(JSON.parse(env.DATASET_COVERAGE_MAP))).toHaveLength(14);
...
describe("syncMcpJson against a copy of the real repo-root mcp.json", () => {
  it("produces the locked 15/5/14 key counts and preserves the real launcher untouched", async () => {
    const fixturePath = join(tmpDir, "mcp.json");
