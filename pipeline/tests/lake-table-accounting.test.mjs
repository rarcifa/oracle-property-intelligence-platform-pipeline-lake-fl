...
describe("clean-runner row-hash recovery", () => {
  it("reconstructs the baseline from the immutable predecessor when the cache is absent", async () => {
    const { mkdtemp } = await import("node:fs/promises");
    const { tmpdir } = await import("node:os");
    const path = (await import("node:path")).default;
    const dir = await mkdtemp(path.join(tmpdir(), "row-hash-recovery-"));
    const attempted = [];

    const hashes = await readPreviousRowHashes(
      path.join(dir, "run-history.json"),
      { rootCid: "bafy-predecessor" },
      {
        gateways: ["https://first.example", "https://second.example/"],
        readHashes: async (url) => {
          attempted.push(url);
          if (url.includes("first.example")) throw new Error("temporarily unavailable");
          return new Map([["parcel-1", "hash-1"]]);
        },
      },
    );

    expect(attempted).toEqual([
      "https://first.example/ipfs/bafy-predecessor/query-table.parquet",
      "https://second.example/ipfs/bafy-predecessor/query-table.parquet",
    ]);
    expect([...hashes]).toEqual([["parcel-1", "hash-1"]]);
  });

  it("fails closed when a recorded predecessor cannot be read", async () => {
    const { mkdtemp } = await import("node:fs/promises");
    const { tmpdir } = await import("node:os");
    const path = (await import("node:path")).default;
    const dir = await mkdtemp(path.join(tmpdir(), "row-hash-recovery-"));
    ...
    await expect(
      readPreviousRowHashes(
        ...
        { rootCid: "bafy-predecessor" },
        {
          gateways: ["https://only.example"],
          readHashes: async () => {
            throw new Error("not found");
          },
        },
      ),
    ).rejects.toThrow(/Could not reconstruct row hashes/);
  });
});
