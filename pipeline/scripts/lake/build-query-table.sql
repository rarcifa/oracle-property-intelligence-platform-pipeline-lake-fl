...
--
-- Parameters are substituted by the driver: $DOWNLOAD_DIR, $OUT_PARQUET,
-- $PERMIT_OUT_PARQUET, $AS_OF_YEAR, $AS_OF_DATE.
...
CREATE OR REPLACE VIEW permit AS
  SELECT permit_number, alternate_key, parcel_id, permit_type, permit_desc, permit_status,
  ...
  SELECT permit_number, alternate_key, parcel_id, permit_type, permit_desc, permit_status,
         applied_date, approved_date, issued_date, co_date, last_modified,
         permit_url, is_roofing, is_open, days_open,
         'lake_clermont_etrakit_permits' AS source_system,
...
SELECT
  ...
  max(CASE WHEN lower(is_open)='true'
      THEN CASE
        WHEN TRY_CAST(coalesce(issued_date, applied_date) AS DATE) IS NULL THEN NULL
        -- A source can pre-date a permit in the future. Preserve that source
        -- date, but never turn the source anomaly into a negative duration.
        WHEN TRY_CAST(coalesce(issued_date, applied_date) AS DATE) > TRY_CAST('$AS_OF_DATE' AS DATE) THEN 0
        ELSE date_diff('day', TRY_CAST(coalesce(issued_date, applied_date) AS DATE), TRY_CAST('$AS_OF_DATE' AS DATE))
      END
      END)                                                                 AS longest_open_permit_days,
  max(CASE WHEN lower(is_open)='true' AND lower(is_roofing)='true'
      THEN CASE
        WHEN TRY_CAST(coalesce(issued_date, applied_date) AS DATE) IS NULL THEN NULL
        WHEN TRY_CAST(coalesce(issued_date, applied_date) AS DATE) > TRY_CAST('$AS_OF_DATE' AS DATE) THEN 0
        ELSE date_diff('day', TRY_CAST(coalesce(issued_date, applied_date) AS DATE), TRY_CAST('$AS_OF_DATE' AS DATE))
...
-- Full permit records are published beside the parcel-grain query table. This
-- table retains records that do not join the assessed roll and says so in
-- linkage_status; aggregating first and publishing only property rows would
-- make those valid source records impossible to query through DuckDB or MCP.
COPY (
  WITH normalized AS (
    SELECT
      concat_ws(':', p.source_system, p.permit_number)                     AS permit_id,
      nullif(trim(p.permit_number), '')                                    AS permit_number,
      coalesce(nullif(trim(n.PARCEL_ID), ''), nullif(trim(p.parcel_id), '')) AS parcel_identifier,
      nullif(trim(p.alternate_key), '')                                    AS alt_key,
      CASE WHEN p.source_system = 'lake_clermont_etrakit_permits'
           THEN 'City of Clermont' ELSE 'Lake County' END                  AS jurisdiction,
      nullif(trim(p.permit_type), '')                                      AS permit_type,
      nullif(trim(p.permit_desc), '')                                      AS permit_description,
      nullif(trim(p.permit_status), '')                                    AS permit_status,
      nullif(trim(p.applied_date), '')                                     AS applied_date,
      nullif(trim(p.approved_date), '')                                    AS approved_date,
      nullif(trim(p.issued_date), '')                                      AS issued_date,
      nullif(trim(p.co_date), '')                                          AS completed_date,
      nullif(trim(p.last_modified), '')                                    AS last_modified_date,
      TRY_CAST(p.is_roofing AS BOOLEAN)                                    AS is_roofing,
      TRY_CAST(p.is_open AS BOOLEAN)                                       AS is_open,
      CASE
        WHEN TRY_CAST(p.is_open AS BOOLEAN)
          THEN CASE
            WHEN TRY_CAST(coalesce(nullif(p.issued_date, ''), nullif(p.applied_date, '')) AS DATE) IS NULL THEN NULL
            WHEN TRY_CAST(coalesce(nullif(p.issued_date, ''), nullif(p.applied_date, '')) AS DATE) > TRY_CAST('$AS_OF_DATE' AS DATE) THEN 0
            ELSE CAST(date_diff(
              'day',
              TRY_CAST(coalesce(nullif(p.issued_date, ''), nullif(p.applied_date, '')) AS DATE),
              TRY_CAST('$AS_OF_DATE' AS DATE)
            ) AS INTEGER)
          END
        ELSE greatest(0, TRY_CAST(p.days_open AS INTEGER))
      END                                                                  AS days_open,
      nullif(trim(p.contractor_name), '')                                  AS contractor_name,
      nullif(trim(p.contractor_license), '')                               AS contractor_license,
      CAST(NULL AS VARCHAR)                                                AS bbb_rating,
      nullif(trim(p.permit_url), '')                                       AS source_url,
      p.source_system                                                      AS source_system,
      CASE WHEN n.PARCEL_ID IS NOT NULL THEN 'linked_to_assessed_roll'
           ELSE 'unlinked_to_assessed_roll' END                            AS linkage_status,
      row_number() OVER (
        PARTITION BY p.source_system, p.permit_number
        ORDER BY coalesce(p.last_modified, p.co_date, p.issued_date, p.applied_date, '') DESC
      )                                                                    AS duplicate_rank
    FROM permit p
    LEFT JOIN nal n ON n.ALT_KEY = p.alternate_key
    WHERE nullif(trim(p.permit_number), '') IS NOT NULL
  )
  SELECT
    permit_id, permit_number, parcel_identifier, alt_key, jurisdiction,
    permit_type, permit_description, permit_status, applied_date,
    approved_date, issued_date, completed_date, last_modified_date,
    is_roofing, is_open, days_open, contractor_name, contractor_license,
    bbb_rating, source_url, source_system, linkage_status
  FROM normalized
  WHERE duplicate_rank = 1
  ORDER BY permit_id
) TO '$PERMIT_OUT_PARQUET' (FORMAT PARQUET, COMPRESSION ZSTD);
...
COPY (
  SELECT
    ...
    CAST(p.longest_open_roofing_permit_days AS INTEGER)                     AS longest_open_roofing_permit_days,
