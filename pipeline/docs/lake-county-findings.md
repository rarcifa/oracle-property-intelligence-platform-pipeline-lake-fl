...
**It recurs, and it is degradation rather than death.** The resume pass met the same wall
after about 800 more detail requests, at roughly 0.44 req/s — far gentler than the pass
that first triggered it — so the budget is not a simple rate limit and does not reset
quickly. What the portal actually does was measured on the second occurrence: three
unloaded probes 20 s apart returned **HTTP 200 at 27.2 s, then 1.3 s, then 1.2 s**
time-to-first-byte. The server is shedding load by queueing, not by refusing; a client
...
Plan a long harvest around this: it is not a run that finishes in one sitting, it is a run
that is resumed. Resume is the same command with the same `--job-id`, and skip-existing
makes the completed work a no-op.
