...
export class CountyEnrichmentBatchStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const imageAsset = new ecrAssets.DockerImageAsset(this, "BatchImage", {
      ...
      // The Dockerfile copies only source, entrypoints and build metadata.
      // Excluding mutable data and dependencies here matters even before
      // Docker runs: CDK stages the whole context during synth, and a clean
      // test used to copy several gigabytes of local harvest artifacts and
      // node_modules into /tmp for every stack assertion.
      exclude: [
        "cdk.out/**",
        "data/**",
        "docs/**",
