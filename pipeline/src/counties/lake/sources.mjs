 ...
 * @param {object} [options] - Deterministic normalization options.
 * @param {number} [options.nowMs] - Frozen as-of clock in epoch milliseconds.
 * @returns {{
 ...
 * }} Normalized permit.
 */
export function normalizePermit(feature, options = {}) {
  ...
  let daysOpen = null;
  if (open && start) {
    const elapsed = (options.nowMs ?? Date.now()) - Date.parse(`${start}T00:00:00Z`);
