...
export const LAKE_QUERY_TABLE_SCHEMA_FIELDS = Object.freeze({
  ...
  longest_open_roofing_permit_days: { type: "INT32", optional: true },
...
export function mapJoinedRecordToQueryTableRow(record) {
  ...
  const openRoofingDurations = openRoofing
    .map((permit) => toInteger(permit.days_open))
    .filter((days) => days !== null);
  ...
  return {
    ...
    longest_open_roofing_permit_days:
      openRoofingDurations.length > 0 ? Math.max(...openRoofingDurations) : null,
