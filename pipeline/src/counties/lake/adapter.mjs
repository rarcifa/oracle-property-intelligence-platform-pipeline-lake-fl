...
/**
 * Assert the per-permit publication gate: stable non-null permit ids, no
 * duplicate identities, and exact schema order.
 *
 * @param {string} parquetPath - Permit Parquet path.
 * @returns {Promise<{ rows: number, distinctPermits: number, nullPermitIds: number }>} Passing counters.
 */
export async function assertPermitTableGate(parquetPath) {
  const { stdout } = await execFileAsync("duckdb", [
    "-json",
    "-c",
    `SELECT count(*) AS rows, count(DISTINCT permit_id) AS distinct_permits, ` +
  ...
  const counts = {
    ...
    distinctPermits: Number(row.distinct_permits),
    nullPermitIds: Number(row.null_permit_ids),
  };
  if (counts.nullPermitIds !== 0) {
    throw new Error(`Permit table has ${counts.nullPermitIds} null or empty permit_id values`);
  }
  if (counts.rows !== counts.distinctPermits) {
    throw new Error(
      `Permit table row count ${counts.rows} != distinct permit_id ${counts.distinctPermits}`,
    );
  }
