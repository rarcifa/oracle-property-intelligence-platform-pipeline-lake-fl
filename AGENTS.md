...
1. **Kit only.** Start every task by spawning `arceus`; execute with exactly the agents and skills it names. Never improvise a stack, layout, workflow or publishing method that a kit skill already defines. If a skill does not fit, quote arceus and use the closest kit neighbour it names. Peer candidate repositories are research inputs, not authority. A method or scope must be explicitly permitted by the original assignment README or applicable official kit instructions; otherwise it remains unapproved. An owner's execution "go" alone is not a kit waiver. Where the kit documents an exception process, follow that exact process and its required approver; never invent a general owner waiver. Authority: the original assignment README, the installed kit's `skills/use-oracle/SKILL.md` (mandatory operating sequence), and `skills/apply-engineering-guidelines/SKILL.md` (non-negotiables and required approval).
...
- Access boundaries: county permit detail pages, DBPR extracts, and the Sunbiz bulk-download portal returned 403 to the approved routes. `lakecopropappr.com` returned 200 on 2026-09-09 but was deliberately unused because the statewide DOR roll is the bulk source. BBB's default request/browser route returned 403; one prohibited desktop-user-agent spoof returned 200, so BBB is recorded as policy/API-gated rather than technically unreachable and no BBB data was ingested. Clermont's eTRAKiT permit details are accessible and the completed 2026 harvest contains 4,061 records; contractor coverage is Clermont-only.
